    ...
    <div className={`fp__feature-row${cardSide === 'left' ? ' fp__feature-row--reversed' : ''}`}>
      <div className="fp__feature-text-side">
        <div className="fp__feature-text-inner">
          ...
          <div className="fp__feature-title-row">
            <p className="fp__feature-title">{title}</p>
            {comingSoon && <span className="fp__coming-soon">{comingSoonLabel}</span>}
          </div>
...
export default function BusinessAccount() {
  ...
  const t = useT();
  const content = t('paymentAccounts.business');
  const features = content.features.map((f, i) => ({
    ...f,
    cardSide: FEATURE_SIDES[i],
    comingSoon: i === COMING_SOON_INDEX,
  }));
  const ctaTitle = t('paymentAccounts.ctaTitle');
  const comingSoonLabel = t('common.comingSoon');
